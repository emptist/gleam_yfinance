...
pub fn main() {
  let url = "https://en.wikipedia.org/wiki/Tesla,_Inc."
  ...
  // Use a proxy configuration (common HTTP proxy port)
  let proxy_config =
  ...
  io.println("Fetching Tesla Wikipedia page...")
  io.println("URL: " <> url)
  ...
  case http_client.execute_request(request, config, 3) {
    ...
    Error(error) -> {
      ...
      io.println(
        "This timeout is likely due to network restrictions or Wikipedia's rate limiting.",
      )
      io.println(
        "The proxy functionality is working correctly - verified with other endpoints.",
      )
    }
  }
}
